...
The OMV host itself now resolves `*.kosto.top` to **Cloudflare's public IP**, not `192.168.0.12`. That's fine because the services live on this box and are reachable locally by container name / IP — the host doesn't need the internal view.
